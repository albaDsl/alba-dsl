...
import Alba.Dsl.V1.Bch2026.Contract.Prelude (drop, dup)
import DslDemo.EllipticCurve.Constants (g)
import DslDemo.EllipticCurve.JacobianWindowed
  ( TTable,
    ecMul4,
    ecMul6,
    setupTableM,
...
testLookupTables =
  testGroup
    "Lookup tables"
    [ testCase "Elliptic curve - scalar multiply 4" $
        isTrue (evaluateProg progEllipticCurve4),
      testCase "Elliptic curve - scalar multiply 6" $
        isTrue (evaluateProg progEllipticCurve6),
      testCase "Elliptic curve - scalar multiply 4 - precomp" $
        isTrue (evaluateProg progEllipticCurve4Precomp),
      testCase "Elliptic curve - scalar multiply 6 - precomp" $
        isTrue (evaluateProg progEllipticCurve6Precomp)
    ]

progEllipticCurve4 :: Fn s (s :> TBool)
progEllipticCurve4 = runEnv (g ∘ setupTableM 4 ∘ verifyTestVectors ecMul4)

progEllipticCurve6 :: Fn s (s :> TBool)
progEllipticCurve6 = runEnv (g ∘ setupTableM 6 ∘ verifyTestVectors ecMul6)
...
progEllipticCurve4Precomp =
  runEnv (bytes gTable4 ∘ b2v ∘ verifyTestVectors ecMul4)
...
progEllipticCurve6Precomp =
  runEnv (bytes gTable6 ∘ b2v ∘ verifyTestVectors ecMul6)
