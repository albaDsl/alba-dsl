...
initOpDispatch maxCsDepth =
  begin
    ...
    # ( begin
          # progCode (toTyped (ifOp maxCsDepth id))
          # opDefineIdx 0x63 -- OP_IF
      )
    # ( begin
          # progCode (toTyped (ifOp maxCsDepth UT.opNot))
    ...
    # (progCode toggleCondStack # opDefineIdx 0x67) -- OP_ELSE
    # (progCode dropCondStack # opDefineIdx 0x68) -- OP_ENDIF
    # (bytes' [0x69] # opDefineIdx 0x69) -- OP_VERIFY
    # (bytes' [0x6a] # opDefineIdx 0x6a) -- OP_RETURN
    # (progCode (toTyped toAltStack) # opDefineIdx 0x6b) -- OP_TOALTSTACK
    # (progCode (toTyped fromAltStack) # opDefineIdx 0x6c) -- OP_FROMALTSTACK
